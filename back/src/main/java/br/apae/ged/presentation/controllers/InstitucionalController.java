...
public class InstitucionalController {
    ...
    public ResponseEntity<InstucionalUploadResponse> upload(
            ...
            @Parameter(description = "Localização do documento.") @RequestParam("localizacao") String localizacao,
            @Parameter(description = "Arquivo a ser enviado.") @RequestPart("file") MultipartFile documento) {
        try {
            var response = service
                    .uploadDocumento(new UploadInstitucionalRequest(documento, titulo, dataCriacao, tipoDocumento, localizacao));
            return ResponseEntity.status(HttpStatus.CREATED).body(response);
        } catch (Exception e) {
    ...
    @Operation(summary = "Solicita código de autenticação para assinatura", description = "Inicia o processo de assinatura de um documento institucional, enviando um código de verificação por e-mail.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Código de autenticação enviado com sucesso."),
            @ApiResponse(responseCode = "400", description = "Documento já assinado ou senha inválida.", content = @Content),
            @ApiResponse(responseCode = "404", description = "Documento não encontrado.", content = @Content)
    })
    @PostMapping("/iniciar-assinatura/{id}")
    @PreAuthorize("hasAuthority('DOCUMENTO_INSTITUCIONAL_WRITE')")
    public ResponseEntity<Void> solicitarCodigoAssinatura(
            @Parameter(description = "ID do documento a ser assinado.") @PathVariable Long id,
            @RequestBody UserLoginDTO entrada) {
        service.solicitarCodigoAssinatura(id, entrada);
        return ResponseEntity.ok().build();
    }

    @Operation(summary = "Confirma a assinatura com código de autenticação", description = "Finaliza o processo de assinatura utilizando o código de autenticação recebido por e-mail.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Documento assinado com sucesso."),
            @ApiResponse(responseCode = "400", description = "Código de autenticação inválido ou expirado.", content = @Content)
    })
