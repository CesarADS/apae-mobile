...
const Home = (): ReactElement => {
    ...
    const canWrite = hasPermission('ALUNO_WRITE');
    const canDelete = hasPermission('ALUNO_DELETE');
    ...
    const abrirModalEdicao = async (aluno: Aluno) => { if (!canWrite) return; setAlunoEmEdicao(aluno); setModalFormVisivel(true); setCarregandoModal(true); try { const response = await alunoService.listarUmAluno(aluno.id!); setDadosForm(response); } catch (error) { showAlert("Erro ao buscar dados do aluno.", "Erro!", "error"); fecharModalForm(); } finally { setCarregandoModal(false); } };
        ...
        <Form>
            <fieldset className="border border-primary-subtle rounded p-2 mb-2">
                ...
                <Row className="g-2">
                    ...
                    <Form.Group as={Col} md={4} controlId="matricula"><Form.Label className="form-label-sm">Prontuário<span className="text-danger">*</span></Form.Label><Form.Control size="sm" type="number" name="matricula" value={dadosForm.matricula || ''} onChange={handleFormChange} required /></Form.Group>
            ...
            <div className="d-flex flex-column flex-md-row justify-content-md-between align-items-md-center mb-4 gap-3">
                <div className="flex-grow-1"><h2 className="text-primary">Alunos</h2>
                    <div className="d-flex align-items-center gap-2" style={{ maxWidth: '450px' }}>
                        <Form.Control
                            type="text"
                            placeholder="Pesquisar por nome, Prontuário ou CPF..."
                            value={termoBusca}
                    ...
                    <Table borderless={true} hover responsive>
                        <thead>
                            <tr className="thead-azul">
                                <th className="text-center align-middle" style={{ width: '15%' }}>Prontuário</th>
                        ...
                        <tbody>
                            {erro ? (<tr><td colSpan={5} className="text-center text-danger">Erro ao buscar alunos.</td></tr>) :
                                paginaData?.content && paginaData.content.length > 0 ? (
                                    paginaData.content.map(aluno => (
                                        <tr key={aluno.id} className="border border-primary tr-azul-hover" onClick={() => abrirModalEdicao(aluno)} style={{ cursor: canWrite ? 'pointer' : 'default' }}>
