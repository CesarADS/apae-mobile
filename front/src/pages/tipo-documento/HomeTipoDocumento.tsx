...
const HomeTipoDocumento = (): ReactElement => {
    ...
    const handleFormChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
        ...
        if (type === 'checkbox') {
            ...
            setDadosForm(prev => {
                ...
                if (name === 'aluno' && checked) {
                    newState.colaborador = false;
                    newState.institucional = false;
                }
                if (name === 'colaborador' && checked) {
                    newState.aluno = false;
                    newState.institucional = false;
                }
                if (name === 'institucional' && checked) {
                    newState.aluno = false;
                    newState.colaborador = false;
                }
                return newState;
    ...
    const abrirModalEdicao = (item: TipoDocumentoResponse) => {
        ...
        setDadosForm({
            ...
            documentoAssinavel: item.documentoAssinavel, gerarDocumento: item.gerarDocumento,
            colaborador: item.colaborador,
            aluno: !item.colaborador && !item.institucional
        });
        setModalFormVisivel(true);
    };
    ...
    const handleSalvar = async () => {
        ...
        const payload: TipoDocumentoRequest = {
            ...
            gerarDocumento: dadosForm.gerarDocumento,
            colaborador: dadosForm.colaborador
        };
    ...
    const renderizarFormulario = () => (
        <>
            <fieldset className="border border-primary-subtle rounded p-2 mb-2">
                ...
                <Form>
                    <Form.Group className="mb-3" controlId="nome">
                        <Form.Label className="form-label-md">Nome do tipo de documento <span
                            className="text-danger">*</span></Form.Label>
                        <Form.Control size="md" type="text" name="nome" value={dadosForm.nome}
                                      onChange={handleFormChange} required/>
                    </Form.Group>
                    <Form.Group controlId="validade">
                        <Form.Label className="form-label-md">Prazo de vigência <span
                            className="text-danger">*</span></Form.Label>
                        <InputGroup size="md">
                            <Form.Control type="number" name="valorValidade" value={dadosForm.valorValidade}
                                          onChange={e => {
                            ...
                            <Form.Select name="unidadeValidade" value={dadosForm.unidadeValidade}
                                         onChange={handleFormChange} style={{maxWidth: "120px"}}>
            ...
            <fieldset className="border border-primary-subtle rounded p-2 mb-1">
                <legend className="float-none w-auto px-2 h6 m-0 text-primary">Posse</legend>
                 <Form.Check className="mb-2" type="switch" id="aluno" name="aluno" label="Aluno"
                            checked={dadosForm.aluno} onChange={handleFormChange}
                            disabled={dadosForm.colaborador || dadosForm.institucional}/>
                <Form.Check className="mb-2" type="switch" id="colaborador" name="colaborador" label="Colaborador"
                            checked={dadosForm.colaborador} onChange={handleFormChange}
                            disabled={dadosForm.institucional || dadosForm.aluno}/>
                <Form.Check className="mb-2" type="switch" id="institucional" name="institucional" label="Institucional"
                            checked={dadosForm.institucional} onChange={handleFormChange}
                            disabled={dadosForm.colaborador || dadosForm.aluno}/>
            </fieldset>
            <fieldset className="border border-primary-subtle rounded p-2">
                ...
                <Form.Check className="mb-2" type="switch" id="guardaPermanente" name="guardaPermanente"
                            label="Guarda Permanente" checked={dadosForm.guardaPermanente} onChange={handleFormChange}/>
                <Form.Check className="mb-2" type="switch" id="documentoAssinavel" name="documentoAssinavel"
                            label="Documento Assinável" checked={dadosForm.documentoAssinavel}
                            onChange={handleFormChange}/>
                <Form.Check className="mb-2" type="switch" id="gerarDocumento" name="gerarDocumento"
                            label="Permissão para geração de PDF" checked={dadosForm.gerarDocumento}
                            onChange={handleFormChange}/>
            </fieldset>
        </>
    );
        ...
        <Container fluid>
            <div className="d-flex flex-column flex-md-row justify-content-md-between align-items-md-center mb-4 gap-3">
                ...
                <div className="d-flex flex-wrap justify-content-start justify-content-md-end gap-2 ">
                    <Botao variant="primary" icone={<Icone nome="plus-circle"/>} onClick={abrirModalCadastro}
                           texto="Cadastrar"/>
                </div>
            </div>
                ...
                <>
                    <Table borderless={true} hover responsive>
                        ...
                        <tr>
                            ...
                            <th className="text-center d-none d-md-table-cell" style={{width: '30%'}}>Data de Registro
                            </th>
                        ...
                        {paginaData?.content && paginaData.content.length > 0 ? (
                            paginaData.content.map(item => (
                                <tr key={item.id} className="border border-primary tr-azul-hover"
                                    onClick={() => abrirModalEdicao(item)} style={{cursor: 'pointer'}}>
                                    ...
                                    <td className="text-center align-middle">
                                        <Botao variant="link" className="p-0 text-danger" title="Inativar"
                                               onClick={(e) => handleInativarClick(e, item)}
                                               icone={<Icone nome="ban"/>}/>
                                    </td>
                                </tr>
                            ))
                        ) : (<tr>
                            <td colSpan={4} className="text-center py-4">Nenhum tipo de documento encontrado.</td>
                        </tr>)}
                        </tbody>
                    </Table>

                    <div className="d-flex justify-content-center align-items-center gap-2 mt-4">
                        <Button variant="primary" onClick={() => setPaginaAtual(p => p - 1)}
                                disabled={paginaData?.first}>&larr; Anterior</Button>
                        <span>Página {paginaData ? paginaData.number + 1 : 0} de {paginaData?.totalPages ?? 0}</span>
                        <Button variant="primary" onClick={() => setPaginaAtual(p => p + 1)}
                                disabled={paginaData?.last}>Próxima &rarr;</Button>
                    </div>
                </>
            )}
